...
tuple_frequency = {}
for item in valid_gears:
    _, tpl = item
    tuple_frequency[tpl] = tuple_frequency.get(tpl, 0) + 1
filtered_data = [item for item in valid_gears if tuple_frequency[item[1]] > 1]
...
for gear in filtered_data:
    if couples.get(gear[1]) is not None:
        ...
        couples[gear[1]] = gear[0]
print(f'Sum of all of the part numbers in the engine schematic is {sum(list(couples.values()))}')
